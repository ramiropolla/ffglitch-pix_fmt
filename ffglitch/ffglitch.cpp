...
bool FFglitch::decode_ipkt(AVFrame *iframe, int flags)
{
	bool ok = false;
	...
	const char *const opt_str = "-e_dc-a_dc-i_dc";
	char opt[16];
	strcpy(opt, opt_str);

	if ( (flags & DEC_EXPORT) != 0 )
		opt[0] = '+';
	if ( (flags & DEC_APPLY) != 0 )
		opt[5] = '+';
	if ( (flags & DEC_IMPORT) != 0 )
	{
		opt[10] = '+';
	...
	av_dict_set(&opts, "ffglitch_flags", opt, 0);
...
bool FFglitch::open_image(const char *_fname)
{
	...
	av_dump_format(ctx_ifmt, 0, fname, 0);

	if ( strcmp(ctx_ifmt->iformat->name, "image2") != 0 )
	{
		fprintf(stderr, "Only image files are supported for now\n");
		goto the_end;
	}

	codec_id = ctx_ifmt->streams[0]->codecpar->codec_id;
	if ( codec_id != AV_CODEC_ID_MJPEG )
	{
		fprintf(stderr, "Only AV_CODEC_ID_MJPEG is supported for now\n");
		goto the_end;
	}
	...
	if ( !decode_ipkt(iframe, DEC_EXPORT) )
		goto the_end;
...
bool FFglitch::update_image()
{
	...
	if ( !decode_ipkt(iframe, DEC_IMPORT) )
		goto the_end;
...
bool FFglitch::save_image(const char *_fname)
{
	...
	if ( !decode_ipkt(iframe, DEC_IMPORT | DEC_APPLY) )
		goto the_end;
...
const FFImage *FFglitch::get_image() const
{
	return image;
}
